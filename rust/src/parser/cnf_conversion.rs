...
use crate::core::{Variable, Term, Atom, Literal, Clause, CNFFormula, Constant, FunctionSymbol};
...
impl CNFConverter {
    ...
    fn skolemize(&mut self, formula: FOFFormula) -> FOFFormula {
        match formula {
            ...
            FOFFormula::And(f1, f2) => FOFFormula::And(
                Box::new(self.skolemize(*f1)),
                Box::new(self.skolemize(*f2))
            ),
            ...
            FOFFormula::Quantified(Quantifier::Forall, var, f) => {
                ...
                let result = FOFFormula::Quantified(
                    Quantifier::Forall,
                    var,
                    Box::new(self.skolemize(*f))
                );
    ...
    fn substitute_in_formula(&self, formula: FOFFormula, var: &Variable, term: &Term) -> FOFFormula {
        match formula {
            ...
            FOFFormula::Not(f) => FOFFormula::Not(
    ...
    fn remove_universal_quantifiers(&self, formula: FOFFormula) -> FOFFormula {
        match formula {
            ...
            FOFFormula::Not(f) => FOFFormula::Not(
                Box::new(self.remove_universal_quantifiers(*f))
            ),
            _ => formula,
        }
    }
