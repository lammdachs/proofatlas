...
use crate::parsing::fof::FofFormula;
use crate::core::Problem;
use crate::core::parser_convert::parse_problem_to_array;
...
pub fn parse_file(file_path: &str, include_path: Option<&str>) -> Result<Problem> {
    ...
    let parse_problem = parser_state.parse_file_content(&content, Path::new(file_path))?;
    parse_problem_to_array(parse_problem)
        .map_err(|e| ParseError::ParseError(format!("Failed to convert to array: {}", e)))
}

/// Parse TPTP string content
pub fn parse_string(content: &str) -> Result<Problem> {
    let mut parser_state = ParserState::new(None);
    let parse_problem = parser_state.parse_string_content(content)?;
    parse_problem_to_array(parse_problem)
        .map_err(|e| ParseError::ParseError(format!("Failed to convert to array: {}", e)))
}
...
impl ParserState {
    ...
    fn parse_file_content(&mut self, content: &str, file_path: &Path) -> Result<ParseProblem> {
    ...
    fn parse_string_content(&mut self, content: &str) -> Result<ParseProblem> {
        let inputs = parse_tptp_inputs(content)
            .map_err(|e| ParseError::ParseError(format!("Parse error: {:?}", e)))?;
        ...
        for input in inputs {
            match input {
                ...
                TPTPInput::Fof { name: _, role, formula } => {
                    // Convert FOF to CNF
        ...
        for (idx, clause) in self.clauses.iter().enumerate() {
            let is_conjecture = self.conjecture_indices.contains(&idx);
            problem.add_clause(clause.clone(), is_conjecture);
        }
        Ok(problem)
    }
}
    ...
    while !remaining.trim().is_empty() {
        ...
        match parse_tptp_input(remaining) {
            ...
            Err(e) => {
                ...
                } else if remaining.starts_with("cnf(") || remaining.starts_with("fof(") {
                    // This is a CNF/FOF that failed to parse - return error
                    return Err(ParseError::ParseError(format!(
                        "Failed to parse TPTP input: {:?}", e
                    )));
                } else {
                    // Skip to next line if it's not a recognized formula type
                    if let Some(pos) = remaining.find('\n') {
                        remaining = &remaining[pos + 1..];
                    } else {
                        break;
...
fn parse_clause(input: &str) -> IResult<&str, ParseClause> {
    alt((
        // Empty clause
        map(tag("$false"), |_| ParseClause { literals: vec![] }),
        // Parenthesized clause
        delimited(
            ws(char('(')),
            map(
                separated_list1(ws(char('|')), parse_literal),
                |lits| ParseClause { literals: lits }
            ),
            ws(char(')'))
        ),
        // Disjunction of literals (with or without parentheses)
        map(
            separated_list1(ws(char('|')), parse_literal),
            |lits| ParseClause { literals: lits }
        ),
...
/// Parse a literal
fn parse_literal(input: &str) -> IResult<&str, ParseLiteral> {
    alt((
        // Negative literal
        map(
            preceded(ws(char('~')), parse_atom_or_equality),
            |pred| ParseLiteral { predicate: pred, polarity: false }
        ),
        // Inequality (A != B becomes ~(A = B))
        map(parse_inequality, |pred| ParseLiteral { predicate: pred, polarity: false }),
        // Positive literal (including equality)
        map(parse_atom_or_equality, |pred| ParseLiteral { predicate: pred, polarity: true }),
    ))(input)
}

/// Parse atom or equality
fn parse_atom_or_equality(input: &str) -> IResult<&str, ParsePredicate> {
    alt((
        parse_equality,
        parse_atom,
    ))(input)
...
fn parse_equality(input: &str) -> IResult<&str, ParsePredicate> {
    let (input, left) = parse_term(input)?;
    let (input, _) = ws(char('='))(input)?;
    let (input, right) = parse_term(input)?;
    
    Ok((input, ParsePredicate { name: "=".to_string(), args: vec![left, right] }))
}

/// Parse inequality (returns equality predicate, caller will negate)
fn parse_inequality(input: &str) -> IResult<&str, ParsePredicate> {
    let (input, left) = parse_term(input)?;
...
fn parse_atom(input: &str) -> IResult<&str, ParsePredicate> {
    ...
    Ok((input, ParsePredicate { name, args: args.unwrap_or_default() }))
}

/// Parse term
fn parse_term(input: &str) -> IResult<&str, ParseTerm> {
    alt((
        // Variable (uppercase)
        map(parse_variable, |name| ParseTerm::Variable(name)),
...
fn parse_function_or_constant(input: &str) -> IResult<&str, ParseTerm> {
    let (input, name) = parse_name(input)?;
    let (input, args) = opt(delimited(
        ws(char('(')),
        separated_list1(ws(char(',')), parse_term),
    ...
    Ok((input, match args {
        Some(args) => ParseTerm::Function { name, args },
        None => ParseTerm::Constant(name),
    }))
}
...
"#;
        ...
        // TODO: Problem doesn't track conjecture indices separately
    }
    ...
    fn test_fof_to_cnf_conversion() {
        ...
        let polarities: Vec<_> = literals.iter().map(|&lit| problem.node_polarities[lit]).collect();
        assert!(polarities.contains(&1));
        assert!(polarities.contains(&-1));
    }
