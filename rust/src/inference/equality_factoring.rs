...
use crate::core::{Clause, Literal, Atom, PredicateSymbol, KBO, KBOConfig, TermOrdering as Ordering};
...
pub fn equality_factoring(clause: &Clause, idx: usize, selector: &dyn LiteralSelector) -> Vec<InferenceResult> {
    ...
    let positive_eq_literals: Vec<(usize, &Literal)> = clause.literals.iter()
        .enumerate()
