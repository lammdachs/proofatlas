...
use crate::core::{Term, Substitution};
...
fn match_with_subst(pattern: &Term, term: &Term, subst: &mut Substitution) -> Result<(), UnificationError> {
    match (pattern, term) {
...
mod tests {
    use super::*;
    use crate::core::{Constant, Variable, FunctionSymbol};
    ...
    fn test_no_match_inconsistent_variable() {
        ...
        let inv = FunctionSymbol { name: "inv".to_string(), arity: 1 };
        let x = Term::Variable(Variable { name: "X".to_string() });
        let y = Term::Variable(Variable { name: "Y".to_string() });
