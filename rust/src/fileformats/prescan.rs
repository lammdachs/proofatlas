...
use crate::core::error::{ProofAtlasError, Result};
...
pub fn prescan_file(
    ...
    let canonical_path = file_path.canonicalize()
        .map_err(|_| ProofAtlasError::IncludeNotFound(file_path.display().to_string()))?;
    let path_str = canonical_path.display().to_string();
    
    if visited.contains(&path_str) {
        return Err(ProofAtlasError::CircularInclude(path_str));
    }
    
    if max_depth <= 0 {
        return Err(ProofAtlasError::DepthLimitExceeded);
    }
...
fn resolve_include_path(current_file: &Path, include_path: &str) -> Result<std::path::PathBuf> {
    let parent = current_file.parent()
        .ok_or_else(|| ProofAtlasError::InvalidInput("Invalid file path".to_string()))?;
    ...
    Err(ProofAtlasError::IncludeNotFound(include_path.to_string()))
}
