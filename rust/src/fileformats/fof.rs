...
impl FofFormula {
    ...
    fn substitute(&self, subst: &HashMap<String, Term>) -> FofFormula {
        match self {
            FofFormula::Atom(p) => {
                let new_args = p.args.iter().map(|term| {
                    substitute_term(term, subst)
                }).collect();
                FofFormula::Atom(Predicate::new(p.name.clone(), new_args))
            },
    ...
    fn extract_clauses(&self) -> Vec<Clause> {
        match self {
            FofFormula::And(fs) => {
            ...
            FofFormula::Not(f) => {
                if let FofFormula::Atom(p) = f.as_ref() {
                    vec![Clause::new(vec![Literal::negative(p.clone())])]
                } else {
                    panic!("Not should only be applied to atoms in CNF")
                }
            },
            FofFormula::Atom(p) => {
                vec![Clause::new(vec![Literal::positive(p.clone())])]
            },
    ...
    fn collect_literals_from_or(&self, literals: &mut Vec<Literal>) {
        match self {
            FofFormula::Or(fs) => {
            ...
            FofFormula::Atom(p) => {
                literals.push(Literal::positive(p.clone()));
            },
            FofFormula::Not(f) => {
                if let FofFormula::Atom(p) = f.as_ref() {
                    literals.push(Literal::negative(p.clone()));
                } else {
                    panic!("Not should only be applied to atoms in CNF");
                }
            },
    ...
    /// Convert a clause-form formula directly to a Clause
    fn to_clause(&self) -> Clause {
        match self {
            FofFormula::Atom(p) => Clause::new(vec![Literal::positive(p.clone())]),
            FofFormula::Not(f) => {
                if let FofFormula::Atom(p) = f.as_ref() {
                    Clause::new(vec![Literal::negative(p.clone())])
                } else {
    ...
    fn collect_literals_from_flat_or(&self, literals: &mut Vec<Literal>) {
        match self {
            FofFormula::Atom(p) => literals.push(Literal::positive(p.clone())),
            FofFormula::Not(f) => {
                if let FofFormula::Atom(p) = f.as_ref() {
                    literals.push(Literal::negative(p.clone()));
                }
    ...
    fn to_literal(&self) -> Literal {
        match self {
            FofFormula::Atom(p) => Literal::positive(p.clone()),
            FofFormula::Not(f) => {
                if let FofFormula::Atom(p) = f.as_ref() {
...
mod tests {
    ...
    fn test_simple_cnf_conversion() {
        ...
        let formula = FofFormula::And(vec![
            FofFormula::Atom(Predicate::new("P".to_string(), vec![])),
            FofFormula::Atom(Predicate::new("Q".to_string(), vec![])),
        ]);
    ...
    fn test_implication_elimination() {
        ...
        let formula = FofFormula::Implies(
            Box::new(FofFormula::Atom(Predicate::new("P".to_string(), vec![]))),
            Box::new(FofFormula::Atom(Predicate::new("Q".to_string(), vec![])))
        );
    ...
    fn test_skolemization() {
        ...
        let formula = FofFormula::Exists(
            vec!["X".to_string()],
            Box::new(FofFormula::Atom(Predicate::new("p".to_string(), vec![Term::Variable("X".to_string())])))
        );
        ...
        match &pred.args[0] {
            Term::Constant(name) => assert!(name.starts_with("sk")),
        ...
        let formula = FofFormula::Forall(
            ...
            Box::new(FofFormula::Implies(
                Box::new(FofFormula::And(vec![
                    FofFormula::Atom(Predicate::new("p".to_string(), vec![Term::Variable("X".to_string())])),
                    FofFormula::Atom(Predicate::new("q".to_string(), vec![Term::Variable("Y".to_string())])),
                ])),
                Box::new(FofFormula::Or(vec![
                    FofFormula::Atom(Predicate::new("r".to_string(), vec![Term::Variable("X".to_string())])),
                    FofFormula::Atom(Predicate::new("s".to_string(), vec![Term::Variable("Y".to_string())])),
                ]))
            ))
        );
