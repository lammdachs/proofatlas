...
use proofatlas::selection::{AgeBasedSelector, AgeWeightRatioSelector, SizeBasedSelector};
use proofatlas::{
...
fn main() {
    ...
    let strategies: Vec<(
        &str,
        Box<dyn Fn() -> Box<dyn proofatlas::selection::ClauseSelector>>,
    )> = vec![
        ("Age-Based (FIFO)", Box::new(|| Box::new(AgeBasedSelector))),
        ("Size-Based", Box::new(|| Box::new(SizeBasedSelector))),
        (
            "Age-Weight 1:5",
            Box::new(|| Box::new(AgeWeightRatioSelector::new(1, 5))),
        ),
        (
            "Age-Weight 1:1",
            Box::new(|| Box::new(AgeWeightRatioSelector::new(1, 1))),
        ),
        (
            "Age-Weight 1:10",
            Box::new(|| Box::new(AgeWeightRatioSelector::new(1, 10))),
        ),
    ];
