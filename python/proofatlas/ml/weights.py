...
def get_encoder_type(preset: dict) -> Optional[str]:
    ...
    Returns "graph", "string", "features", or None for non-ML presets.
    """
...
def find_weights(weights_dir: Path, preset: dict, preset_name: str) -> Optional[Path]:
    ...
    Args:
        ...
        preset_name: Preset name used as model file name
    ...
    if encoder_type in ("graph", "features"):
        if (weights_dir / f"{preset_name}.pt").exists():
            return weights_dir
    elif encoder_type == "string":
        model_path = weights_dir / f"{preset_name}.pt"
        tokenizer_path = weights_dir / f"{preset_name}_tokenizer" / "tokenizer.json"
