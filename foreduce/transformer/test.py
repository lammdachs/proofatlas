...
class UpdateConfig(unittest.TestCase):
    ...
    def init_embedding(self):
        return FormulaEmbedding(TokenConfig(
            num_functions=[i for i in self.num_functions],
    ...
    def test_update_variables(self):
        ...
        for i in range(self.num_variables, cfg.num_variables):
            self.assertEqual(
                float(embedding.embeddings(torch.tensor([cfg.RESERVED_TOKENS + sum(cfg.num_functions) + i], dtype=int))),
                float(embedding.embeddings(torch.tensor([cfg.RESERVED_TOKENS + sum(cfg.num_functions) + self.num_variables], dtype=int)))
            )
    ...
    def test_update_functions_1(self):
        ...
        for i in range(len(self.num_functions)):
            for j in range(self.num_functions[i], cfg.num_functions[i]):
                self.assertEqual(
                    float(embedding.embeddings(torch.tensor([cfg.RESERVED_TOKENS + sum(cfg.num_functions[:i]) + j], dtype=int))),
                    float(embedding.embeddings(torch.tensor([cfg.RESERVED_TOKENS + sum(cfg.num_functions[:i])], dtype=int)))
                )
    ...
    def test_update_functions_2(self):
        ...
        for i in range(sum(cfg.num_functions[len(self.num_functions):])):
            self.assertEqual(
                float(embedding.embeddings.weight[cfg.RESERVED_TOKENS + sum(self.num_functions) + i]),
                float(embedding.embeddings.weight[cfg.RESERVED_TOKENS + sum(self.num_functions) - 1])
            )
