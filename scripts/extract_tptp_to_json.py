#!/usr/bin/env python3
"""
Extract all parsable TPTP problems and save them as JSON.
...
def parse_single_file(file_info: Tuple[Path, Path]) -> Tuple[str, bool, Any]:
    """
    Parse a single TPTP file.
    
    Args:
        file_info: Tuple of (file_path, tptp_root)
    
    Returns:
        Tuple of (relative_path, success, result)
        If success is True, result is the parsed problem dict
        If success is False, result is the error message
    """
    file_path, tptp_root = file_info
    ...
    try:
        ...
        return (relative_path, True, problem_dict)
    except Exception as e:
        return (relative_path, False, str(e))


def parse_files_parallel(tptp_files: List[Path], tptp_root: Path, num_workers: int = None) -> Tuple[List[Dict], List[Dict]]:
    ...
    # Create list of (file_path, tptp_root) tuples
    file_infos = [(f, tptp_root) for f in tptp_files]
    ...
    for relative_path, success, result in results:
        if success:
    ...
    return parsed_problems, failed_files


def main():
    parser = argparse.ArgumentParser(description='Extract TPTP problems to JSON')
    ...
    parsed_problems, failed_files = parse_files_parallel(tptp_files, tptp_root, args.workers)
    
    # Save parsed problems in batches
